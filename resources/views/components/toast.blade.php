...
@php
    // === NUEVO: Derivar nombre de rol desde distintas claves de sesión sin romper tu lógica ===
    // Prioridad: login_role (int) / role (int|str) / rol_id (int) / rol_name (str)
    $roleCode = session('login_role') ?? session('role') ?? session('rol_id') ?? null;
    $roleName = session('login_role_name') ?? session('rol_name') ?? null;

    // Si solo viene código, mapea a nombre
    if (!$roleName && $roleCode !== null) {
        $roleName = match((int) $roleCode) {
            1 => 'Usuario',
            2 => 'Coordinador de Posgrado',
            3 => 'Administrador del Sistema',
            default => 'Usuario',
        };
    }

    // Si tenemos nombre de rol, armamos un toast propio (tipo success)
    $roleToast = $roleName
        ? ['text' => 'Acceso correcto como ' . $roleName, 'type' => 'success']
        : null;

    // Reúne mensajes desde la sesión (rol, success, status, warning, danger, error)
    // OJO: Mantenemos tu session('rol') original tal cual.
...
>
    <div class="flex w-full max-w-sm flex-col gap-2">
        <template x-for="(t, i) in toasts" :key="i">
            <div
                x-cloak
                x-init="setTimeout(() => remove(i), 5000)" {{-- ⏱️ autocierra en 5s --}}
                x-show="true"
