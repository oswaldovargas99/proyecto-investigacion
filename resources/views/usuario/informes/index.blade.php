...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 mt-20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <a href="{{ route('usuario.informes.create') }}"
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4">
            ...
            <div class="mb-4 rounded-xl border bg-white p-4">
                <form method="GET" action="{{ route('usuario.informes.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($informes as $informe)
                            ...
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-4 py-3">
                                    @if(method_exists($informes,'firstItem'))
                                        {{ $loop->iteration + ($informes->firstItem() - 1) }}
                                    @else
                                        {{ $loop->iteration }}
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <span class="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium
                                        ...
                                        {{ optional($fecha)->format('d/m/Y') ?: '—' }}
                                    </span>
                                </td>

                                <td class="px-4 py-3">
                                    @if($categoriaMejoraLbl && $categoriaMejoraLbl !== '—')
                                ...
                                <td class="px-4 py-3">
                                    @if($criterioSnpLbl && $criterioSnpLbl !== '—')
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    {{ is_numeric($m) ? '$'.number_format($m, 2, '.', ',') : '—' }}
                                </td>

                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <a href="{{ route('usuario.informes.show', $informe) }}"
                                           title="Ver"
                                        ...
                                        <a href="{{ route('usuario.informes.edit', $informe) }}"
                                           title="Editar"
                                                ...
                                                <div class="relative w-full max-w-sm rounded-xl bg-white p-5 shadow-lg">
                                                    ...
                                                    <p class="mb-4 text-sm text-gray-600">
                                                        ...
                                                        <span class="font-medium">
                                                            {{ optional(\Illuminate\Support\Carbon::parse($informe->fecha ?? null))->format('d/m/Y') ?: '—' }}
                                                        </span>?
                                                    </p>
                                                    <div class="flex justify-end gap-2">
                                                        ...
                                                        <form action="{{ route('usuario.informes.destroy', $informe) }}"
                                                              method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
            ...
            <div class="mt-4 flex flex-col items-center justify-between gap-2 sm:flex-row">
                ...
                <div>
                    @if(method_exists($informes ?? null,'onEachSide'))
