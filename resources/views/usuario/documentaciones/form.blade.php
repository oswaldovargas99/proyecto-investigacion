...
<div class="rounded-2xl border bg-white shadow-sm">
    <div class="p-5" x-data="accordionPage()">
        ...
        {{-- ====== FORM PRINCIPAL ====== --}}
        <form id="doc-form"
              action="{{ route('usuario.documentaciones.store') }}"
              method="POST"
            ...
            @foreach($grupos as $grupo => $keys)
                ...
                <section class="rounded-xl border bg-white" x-data="{ open: true }">
                    <button type="button"
                            ...
                            @click="open = !open">
                        <div class="flex items-center gap-3">
                            <div class="h-8 w-8 rounded-full {{ $ui['bg'] }} {{ $ui['text'] }} ring-1 ring-inset {{ $ui['ring'] }} flex items-center justify-center">
                            ...
                            <div>
                                <h4 class="text-sm font-semibold text-gray-800">{{ $grupo }}</h4>
                                <p class="text-xs text-gray-500">Documentos: {{ implode(', ', array_map(fn($k) => $campos[$k], $keys)) }}</p>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 gap-y-6 gap-x-10 md:grid-cols-2">
                                @foreach ($keys as $name)
                                    ...
                                    <div class="col-span-1">
                                        ...
                                        {{-- PREVIEW si hay archivo existente --}}
                                            ...
                                            <div class="rounded-lg border bg-gray-50">
                                                <div class="flex items-center justify-between px-3 py-2">
                                                    ...
                                                    <div class="flex flex-wrap items-center gap-2">
                                                        @if($rutaActual)
                                                            <a href="{{ $rutaActual }}" target="_blank" rel="noopener"
                                                            ...
                                                            <a href="{{ $rutaActual }}" target="_blank" rel="noopener" download
                                                ...
                                                <div class="border-t relative">
                                                    @if($rutaActual)
                                                        <iframe src="{{ $rutaActual }}" class="w-full" style="height: 320px;" data-pdf-frame loading="lazy"></iframe>

                                                        {{-- Fallback visual si el servidor no permite embed --}}
                                        ...
                                        {{-- INPUT para cargar/reemplazar (opcional; valida tipo/size si se elige) --}}
                                        <div id="wrap-{{ $name }}" class="{{ $tieneArchivo ? 'hidden' : '' }}">
                                            ...
                                            <label for="input-{{ $name }}"
                                                ...
                                                <p id="hint-{{ $name }}" class="mt-1 text-xs text-gray-500">Solo PDF • Máx. 5 MB</p>
                                            ...
                                            <div class="mt-2 flex flex-wrap items-center gap-2">
                                                <button type="button" data-clear data-field="{{ $name }}"
        ...
        {{-- Formularios de eliminación (uno por campo) --}}
