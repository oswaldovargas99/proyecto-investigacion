...
<div class="rounded-2xl border bg-white shadow-sm">
    <div x-data="{ a1:true, a2:true }" class="space-y-4 p-4">
        ...
        <section class="rounded-xl border bg-white overflow-hidden">
            ...
            <div x-show="a1" x-collapse class="p-5 border-t">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="fecha_solicitud" id="fecha_solicitud"
                            ...
                            class="w-full rounded-lg border-gray-300 px-3 py-2 text-sm
                                    ...
                                    @error('fecha_solicitud') !border-red-500 !ring-red-500 @enderror">
                    ...
                    {{-- Objetivo (contador con Alpine) --}}
                    <div class="md:col-span-2" x-data="{ text: @js($v('objetivo')), max: 500 }">
                        ...
                        <div class="mt-1 text-right text-xs text-gray-500" aria-live="polite">
                            <span data-count x-text="(text || '').length">{{ mb_strlen($v('objetivo')) }}</span>/<span x-text="max">500</span>
                        </div>
                        @error('objetivo') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    {{-- Justificación (contador con Alpine) --}}
                    <div class="md:col-span-2" x-data="{ text: @js($v('justificacion')), max: 500 }">
                        ...
                        <div class="mt-1 text-right text-xs text-gray-500" aria-live="polite">
                            <span data-count x-text="(text || '').length">{{ mb_strlen($v('justificacion')) }}</span>/<span x-text="max">500</span>
                        </div>
                        @error('justificacion') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    {{-- Resultados esperados (contador con Alpine) --}}
                    <div class="md:col-span-2" x-data="{ text: @js($v('resultados_esperados')), max: 500 }">
                        ...
                        <div class="mt-1 text-right text-xs text-gray-500" aria-live="polite">
                            <span data-count x-text="(text || '').length">{{ mb_strlen($v('resultados_esperados')) }}</span>/<span x-text="max">500</span>
                        </div>
        ...
        {{-- Acciones --}}
        <div class="pt-2">
            @php $isUpdate = isset($solicitud) && $solicitud->exists; @endphp
