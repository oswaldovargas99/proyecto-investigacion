    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4">
            ...
            <div class="mb-4 rounded-xl border bg-white p-4">
                <form method="GET" action="{{ route('admin.coordinadores.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span class="font-semibold">{{ $coordinadores->firstItem() ?? 0 }}–{{ $coordinadores->lastItem() ?? 0 }}</span>
                        <span class="hidden sm:inline">de</span>
                        <span class="font-semibold">{{ $coordinadores->total() }}</span>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($coordinadores as $coordinador)
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-4 py-3">
                                    {{ $loop->iteration + ($coordinadores->firstItem() - 1) }}
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900">{{ $coordinador->nombre_coordinador }}</div>
                                    <div class="text-xs text-gray-500">
                                        {{ optional($coordinador->genero)->genero }} · {{ optional($coordinador->nivelEstudios)->nivel_estudios }}
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <a href="mailto:{{ $coordinador->correo_institucional }}"
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @if($coordinador->extension)
                                        <span class="text-gray-500"> · ext. {{ $coordinador->extension }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                            ...
                                            <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
                                                ...
                                                <div class="relative w-full max-w-sm rounded-xl bg-white p-5 shadow-lg">
                                                    ...
                                                    <p class="mb-4 text-sm text-gray-600">
                                                        ¿Deseas eliminar al coordinador <span class="font-medium">{{ $coordinador->nombre_coordinador }}</span>?
                                                    </p>
                                                    <div class="flex justify-end gap-2">
                                                        ...
                                                        <form action="{{ route('admin.coordinadores.destroy', $coordinador) }}"
                                                              method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
            ...
            <div class="mt-4 flex flex-col items-center justify-between gap-2 sm:flex-row">
                <div class="text-sm text-gray-600">
                    Mostrando
                    <span class="font-semibold">{{ $coordinadores->firstItem() ?? 0 }}</span>
                    a
                    <span class="font-semibold">{{ $coordinadores->lastItem() ?? 0 }}</span>
                    de
                    <span class="font-semibold">{{ $coordinadores->total() }}</span>
                    registros
                </div>
                <div>
                    {{ $coordinadores->onEachSide(1)->links('vendor.pagination.pretty') }}
                </div>
            </div>
