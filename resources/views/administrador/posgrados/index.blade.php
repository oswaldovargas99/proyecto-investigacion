...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 mt-20 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-stretch gap-2 flex-wrap">
            <a href="{{ route('admin.posgrados.create') }}"
            ...
            <form action="{{ route('admin.posgrados.import') }}" method="POST" enctype="multipart/form-data" class="inline-flex">
                @csrf
            ...
            <a href="{{ route('admin.posgrados.plantilla') }}"
        ...
        <div class="mx-auto max-w-7xl px-4">
            ...
            @if(session('success') || session('warning') || session('danger'))
                ...
                <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 5000)" x-show="show"
                     ...
                     class="mb-4 flex items-center gap-2 rounded-lg border px-4 py-3 bg-{{ $type }}-50 text-{{ $type }}-800 border-{{ $type }}-200">
            ...
            <div class="mb-4 rounded-xl border bg-white p-4">
                <form method="GET" action="{{ route('admin.posgrados.index') }}"
                    ...
                    <div class="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span class="font-semibold">{{ $posgrados->firstItem() ?? 0 }}–{{ $posgrados->lastItem() ?? 0 }}</span>
                        <span class="hidden sm:inline">de</span>
                        <span class="font-semibold">{{ $posgrados->total() }}</span>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($posgrados as $posgrado)
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-4 py-3">
                                    {{ $loop->iteration + ($posgrados->firstItem() - 1) }}
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <p class="max-w-[480px] font-medium text-gray-900 leading-5 line-clamp-2" title="{{ $posgrado->nombre_posgrado }}">
                                        {{ $posgrado->nombre_posgrado }}
                                    </p>
                                </td>
                                <td class="px-4 py-3">
                                    {{ optional($posgrado->centroUniversitario)->centro_universitario ?? '—' }}
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <a href="{{ route('admin.posgrados.show', $posgrado) }}"
                                        ...
                                        <form action="{{ route('admin.posgrados.destroy', $posgrado) }}" method="POST" onsubmit="return confirm('¿Eliminar este posgrado?')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
            ...
            <div class="mt-4 flex flex-col items-center justify-between gap-2 sm:flex-row">
                <div class="text-sm text-gray-600">
                    Mostrando
                    <span class="font-semibold">{{ $posgrados->firstItem() ?? 0 }}</span>
                    a
                    <span class="font-semibold">{{ $posgrados->lastItem() ?? 0 }}</span>
                    de
                    <span class="font-semibold">{{ $posgrados->total() }}</span>
                    registros
                </div>
                <div>
                    {{ $posgrados->onEachSide(1)->links('vendor.pagination.pretty') }}
                </div>
            </div>
