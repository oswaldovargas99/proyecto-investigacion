...
<x-app-layout>
    ...
    {{-- Título y botones debajo del header (alineados) --}}
    <div class="max-w-7xl mx-auto px-4 mt-20 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-stretch gap-2 flex-wrap">
            ...
            {{-- CSV Import (ID y name alineados con el Controller) --}}
            <form action="{{ route('admin.centros.import') }}" method="POST" enctype="multipart/form-data" class="inline-flex">
                @csrf
            ...
            {{-- Plantilla --}}
            <a href="{{ route('admin.centros.plantilla') }}"
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4">
            ...
            @if(session('success') || session('warning') || session('danger') || session('error'))
                @php
            ...
            {{-- Error específico del input CSV (sin cambiar lógica) --}}
            @error('csv_file')
            ...
            <div class="mb-4 rounded-xl border bg-white p-4">
                <form method="GET" action="{{ route('admin.centros.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span class="font-semibold">{{ $centros->firstItem() ?? 0 }}–{{ $centros->lastItem() ?? 0 }}</span>
                        <span class="hidden sm:inline">de</span>
                        <span class="font-semibold">{{ $centros->total() }}</span>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($centros as $centro)
                            ...
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-4 py-3">
                                    {{ $centros->firstItem() ? $loop->iteration + ($centros->firstItem() - 1) : $loop->iteration }}
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900">{{ $centro->centro_universitario }}</div>
                                </td>
                                <td class="px-4 py-3">
                                    {{ $centro->nombre_rector ?: '—' }}
                                </td>
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset {{ $c['bg'] }} {{ $c['text'] }} {{ $c['ring'] }}">
                                        {{ $tipo ?: '—' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <a href="{{ route('admin.centros.show', $centro) }}"
                                           title="Ver"
                                            ...
                                            <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
                                                ...
                                                <div class="relative w-full max-w-sm rounded-xl bg-white p-5 shadow-lg">
                                                    ...
                                                    <p class="mb-4 text-sm text-gray-600">
                                                        ¿Deseas eliminar el centro <span class="font-medium">{{ $centro->centro_universitario }}</span>?
                                                    </p>
            ...
            <div class="mt-4 flex flex-col items-center justify-between gap-2 sm:flex-row">
                <div class="text-sm text-gray-600">
                    Mostrando
                    <span class="font-semibold">{{ $centros->firstItem() ?? 0 }}</span>
                    a
                    <span class="font-semibold">{{ $centros->lastItem() ?? 0 }}</span>
                    de
                    <span class="font-semibold">{{ $centros->total() }}</span>
                    registros
                </div>
                <div>
                    {{ $centros->onEachSide(1)->links('vendor.pagination.pretty') }}
                </div>
            </div>
