{{-- resources/views/admin/rubros/form.blade.php --}}
@php
    $r = $rubro ?? null;
    ...
    <section class="rounded-xl border bg-gray-50/50 p-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                @error('concepto') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </div>
        </div>
    </section>
            ...
            <div class="md:col-span-2">
                ...
                >
                    ...
                    @foreach($categorias as $c)
                        <option value="{{ $c->id }}" {{ (string)$v('categoria_id') === (string)$c->id ? 'selected' : '' }}>
                            {{ $c->categoria }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="flex flex-col sm:flex-row gap-3 sm:justify-between sm:items-center">
        ...
        <button type="submit"
            class="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg text-white
                   {{ $esEditar ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-green-600 hover:bg-green-700' }}">
            <!-- save icon -->
            ...
            <span>{{ $esEditar ? 'Actualizar' : 'Guardar' }}</span>
        </button>
    </div>
</div>
