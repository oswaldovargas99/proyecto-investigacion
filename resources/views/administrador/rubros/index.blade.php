...
<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 mt-20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <a href="{{ route('admin.rubros.create') }}"
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4">
            ...
            <div class="mb-4 rounded-xl border bg-white p-4">
                <form method="GET" action="{{ route('admin.rubros.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span class="font-semibold">{{ $rubros->firstItem() ?? 0 }}–{{ $rubros->lastItem() ?? 0 }}</span>
                        <span class="hidden sm:inline">de</span>
                        <span class="font-semibold">{{ $rubros->total() }}</span>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($rubros as $rubro)
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-4 py-3">
                                    {{ $loop->iteration + ($rubros->firstItem() - 1) }}
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900">{{ $rubro->concepto }}</div>
                                </td>

                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium
                                                bg-gray-100 text-gray-700 ring-1 ring-inset ring-gray-200">
                                        {{ $categoriasMap[$rubro->categoria_id] ?? '—' }}
                                    </span>
                                </td>

                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <a href="{{ route('admin.rubros.show', $rubro) }}"
                                           title="Ver"
                                        ...
                                        <a href="{{ route('admin.rubros.edit', $rubro) }}"
                                           title="Editar"
                                            ...
                                            <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
                                                ...
                                                <div class="relative w-full max-w-sm rounded-xl bg-white p-5 shadow-lg">
                                                    ...
                                                    <p class="mb-4 text-sm text-gray-600">
                                                        ¿Deseas eliminar el rubro <span class="font-medium">{{ $rubro->rubro }}</span>?
                                                    </p>
            ...
            <div class="mt-4 flex flex-col items-center justify-between gap-2 sm:flex-row">
                <div class="text-sm text-gray-600">
                    Mostrando
                    <span class="font-semibold">{{ $rubros->firstItem() ?? 0 }}</span>
                    a
                    <span class="font-semibold">{{ $rubros->lastItem() ?? 0 }}</span>
                    de
                    <span class="font-semibold">{{ $rubros->total() }}</span>
                    registros
                </div>
                <div>
                    {{ $rubros->onEachSide(1)->links('vendor.pagination.pretty') }}
                </div>
            </div>
