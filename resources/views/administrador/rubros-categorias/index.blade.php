...
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-lg sm:text-xl font-semibold text-gray-800">
            {{-- vacío intencionalmente para replicar el patrón visual --}}
        </h2>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto px-4 mt-20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <a href="{{ route('admin.rubros-categorias.create') }}"
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4">
            ...
            {{-- Toolbar: búsqueda + contador + limpiar --}}
            <div class="mb-4 rounded-xl border bg-white p-4">
                <form method="GET" action="{{ route('admin.rubros-categorias.index') }}"
                    ...
                    <div class="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span class="font-semibold">{{ $categorias->firstItem() ?? 0 }}–{{ $categorias->lastItem() ?? 0 }}</span>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($categorias as $c)
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-4 py-3">
                                    {{ $loop->iteration + ($categorias->firstItem() - 1) }}
                                </td>

                                <td class="px-4 py-3">
                                    <div class="font-medium text-gray-900">{{ $c->categoria }}</div>
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <a href="{{ route('admin.rubros-categorias.show', $c) }}"
                                           title="Ver"
                                        ...
                                        <a href="{{ route('admin.rubros-categorias.edit', $c) }}"
                                           title="Editar"
                                            ...
                                            <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
                                                ...
                                                <div class="relative w-full max-w-sm rounded-xl bg-white p-5 shadow-lg">
                                                    ...
                                                    <p class="mb-4 text-sm text-gray-600">
                                                        ¿Deseas eliminar la categoría <span class="font-medium">{{ $c->categoria }}</span>?
                                                    </p>
                                                    <div class="flex justify-end gap-2">
                                                        ...
                                                        <form action="{{ route('admin.rubros-categorias.destroy', $c) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
            ...
            <div class="mt-4 flex flex-col items-center justify-between gap-2 sm:flex-row">
                <div class="text-sm text-gray-600">
                    Mostrando
                    <span class="font-semibold">{{ $categorias->firstItem() ?? 0 }}</span>
                    a
                    <span class="font-semibold">{{ $categorias->lastItem() ?? 0 }}</span>
                    de
                    <span class="font-semibold">{{ $categorias->total() }}</span>
                    registros
                </div>
                <div>
                    {{ $categorias->onEachSide(1)->links('vendor.pagination.pretty') }}
                </div>
            </div>
